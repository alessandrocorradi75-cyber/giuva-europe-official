...
import { italyBrand, italyCtas, italyNavItems, italySocialLinks } from "@/data/italia";
...
export function Navbar() {
  const primaryNav = italyNavItems.filter((item) => ["/", "/chi-siamo", "/programmi", "/europa", "/volontariato", "/partner"].includes(item.href));
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-slate-200/70 bg-white/96 shadow-sm backdrop-blur-xl">
      <nav className="mx-auto flex max-w-7xl items-center justify-between gap-5 px-5 py-3">
        <Link href="/" className="flex min-w-0 items-center gap-3" aria-label="GIUVA APS Italia home">
          ...
          <span className="min-w-0 leading-tight">
            <span className="block text-lg font-black tracking-tight text-[#081f3a]">{italyBrand.name}</span>
        ...
        <details className="group lg:hidden">
          ...
          <div className="absolute left-0 right-0 top-[73px] grid gap-1 border-b border-slate-200 bg-white px-5 py-4 shadow-xl">
            <div className="mb-2 flex gap-2">
              <Link href="/" className="rounded-full bg-[#081f3a] px-3 py-1 text-xs font-black text-white">IT</Link>
              <Link href="/en" className="rounded-full bg-slate-100 px-3 py-1 text-xs font-black text-slate-700">EN</Link>
            </div>
        ...
        <div className="hidden items-center gap-3 lg:flex">
          <div className="flex items-center gap-1 rounded-full border border-slate-200 bg-slate-50 p-1 text-xs font-black text-slate-600">
            <Link href="/" className="rounded-full bg-white px-2.5 py-1 text-[#081f3a] shadow-sm">IT</Link>
            <Link href="/en" className="px-2.5 py-1 transition hover:text-[#081f3a]">EN</Link>
          </div>
          <div className="flex items-center gap-1">
            {italySocialLinks.slice(0, 2).map((link) => (
              <a
                key={link.href}
                href={link.href}
                target="_blank"
          ...
          <Link href={italyCtas.volunteer.href} className="btn btn-primary">
